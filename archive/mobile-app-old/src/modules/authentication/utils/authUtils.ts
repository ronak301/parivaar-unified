 ...
 * Get it once: normal SMS login → Metro / Flipper Network → `verifyOtp` → `data.userId`.
 * Leave empty to use `testUserId`.
 */
...
function otpBypassAllowed(): boolean {
  ...
  return process.env.EXPO_PUBLIC_ALLOW_OTP_BYPASS === "true";
}
...
export function getDevOtpBypassUserId(): string {
  ...
  const id = process.env.EXPO_PUBLIC_DEV_OTP_BYPASS_USER_ID;
